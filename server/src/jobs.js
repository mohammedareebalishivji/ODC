...
export async function expirePastShifts() {
  ...
  for (const s of open) {
    ...
    if (remaining <= 0) {
      ...
      await notifyUser(s.manager_id, 'Shift closed', 'No one accepted this shift in 12 hours, so it was removed.', 'shift_expired', { shiftId: s.id });
      ...
      await notifyUser(s.manager_id, 'Expiring soon', `Your shift closes in ${mins} min if no one accepts.`, 'expiring_soon', { shiftId: s.id });
    }
  }
